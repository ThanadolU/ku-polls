...
class VoteModelTests(TestCase):
    ...
    def test_one_user_one_vote(self):
        ...
        self.assertEqual(Vote.objects.get(user=self.user, choice__question=question).choice, choice1)
        self.assertEqual(Vote.objects.all().count(), 1)
        self.assertEqual(response.status_code, 302)
        response =  self.client.post(reverse('polls:vote', args=(question.id, )), {'choice': choice2.id})
        self.assertEqual(Vote.objects.get(user=self.user, choice__question=question).choice, choice2)
        self.assertEqual(Vote.objects.all().count(), 1)
        self.assertEqual(response.status_code, 302)
